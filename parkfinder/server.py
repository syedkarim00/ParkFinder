...
def explain_error(exc: Exception) -> str:
    ...
    if "403" in message or "forbidden" in lowered:
        return (
            ...
            "That means Ontario Parks, or a network service in front of it, refused this automated request. "
            "ParkFinder cannot and should not bypass Ontario Parks traffic-protection controls. "
            "Legitimate next steps are to use the official reservation website, contact Ontario Parks to ask about approved data access, "
            "or make a much smaller, slower selected-parks scan if their terms allow automated personal use. "
            "Original error: "
            f"{message}"
        )
    return message
